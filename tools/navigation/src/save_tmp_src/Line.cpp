...
Point Line::getLineIntersection(const Line& line) {
    ...
    double div = det(xDifference, yDifference);
    if (div == 0.0) {
        return {};
    }
    Point detPoint = Point(det(point1, point2), det(line.point1, line.point2), 0);
    return {det(detPoint, xDifference) / div, det(detPoint, yDifference) / div, point1.z};
}

double Line::det(const Point &point1, const Point &point2) {
