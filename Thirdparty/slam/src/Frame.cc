...
//Copy Constructor
    ...
    Frame::Frame(const cv::Mat &descriptors,std::vector<cv::KeyPoint> &keyPoints,const float cols,const float rows, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef, const float &bf,
                 const float &thDepth)
        ...
        mvKeys = std::vector<cv::KeyPoint>(keyPoints.begin(),keyPoints.end());
        if (mvKeys.empty())
            return;
...
// }
    ...
    void Frame::ComputeImageBounds(const float cols,const float rows) {
        if (mDistCoef.at<float>(0) != 0.0) {
