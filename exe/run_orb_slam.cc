...
void runModelAndOrbSlam(std::string &settingPath, bool *stopFlag, std::shared_ptr<pangolin::OpenGlRenderState> &s_cam,
    ...
    int trackingNumberOfFeatures = fSettings["ORBextractor.trackingNumberOfFeatures"];
    int fIniThFAST = fSettings["ORBextractor.iniThFAST"];
    int fMinThFAST = fSettings["ORBextractor.minThFAST"];
    int nLevels = fSettings["ORBextractor.nLevels"];
    std::shared_ptr < ORB_SLAM2::ORBextractor> orbExtractor = std::make_shared<ORB_SLAM2::ORBextractor>(numberOfFeatures, fScaleFactor, nLevels, fIniThFAST,
                                                             fMinThFAST);
    ...
    while (!pangolin::ShouldQuit() && !*stopFlag) {
        *ready = true;
        ...
        if (d_cam.IsShown()) {
           ...
           cv::Mat descriptors;
           orbExtractor->operator()(img, cv::Mat(), keypoints, descriptors);
